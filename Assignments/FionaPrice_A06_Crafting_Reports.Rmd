...
```{r data.summary, echo=FALSE, results='show'}
...
#PM2.5 Table
kable(summary_PM25, digits = 2, #round to 2 digits
      col.names = c("County","$\\mu g/m^3$"), #add column names 
             caption = "Mean Particulates (2.5mm)") #add title
#Ozone Table 
kable(summary_Ozone, digits = 2, #round to 2 digits
      col.names = c("County","$\\mu g/m^3$"), #add column names 
             caption = "Mean Ozone") #add title
...
```{r 2018 boxplot, echo=FALSE, fig.align="left", fig.cap="2018 Monthly Ozone Levels", fig.width = 7, fig.height = 4}
#Change the EPA Month data to a factor and use Month names. 
...
#Create boxplot for 2018 data.
ggplot(Ozone_2018, aes(x = month, y = Ozone)) +
  geom_boxplot()
```

```{r boxplots, echo=FALSE, fig.align="left", fig.cap="2019 Monthly Ozone Levels", fig.width = 7, fig.height = 4}
#Filter data by 2019. 
Ozone_2019 <- filter(epa_data, year == 2019) %>% 
  drop_na(Ozone)

#Create boxplot for 2019 data.
...
## Git Repository
https://github.com/feeprice/EDE_Fall2024
