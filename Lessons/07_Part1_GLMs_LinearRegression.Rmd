...
>  R squared value: depth explains about 30% variance of statistics. P-value suggests
statistically significant results; we can explain irradiance by depth. 
There is a significant negative correlation between irradiance and depth.
...
#This reduced residual standard error and increased R-squared
...
print(irradiancebydepth2)
#se = FALSE gets rid of confidence intervals
...
plot(TPregression)
par(mfrow = c(1,1))
#Clear from R squared that there's something else causing variability of total
#phosphoral concentration, so make a correlation matrix
```
...
#dark red = strong negative correlation, dark blue = strong positive 
corrplot.mixed(PeterPaulCorr, upper = "ellipse")
#prints upper diagonal only 
```
...
summary(TPmodel)
#Removes variable step by step 
#best regression considers oxygen, temp, and nitrogen
